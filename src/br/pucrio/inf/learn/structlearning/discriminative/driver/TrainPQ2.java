...
public class TrainPQ2 implements Command {
	...
	public void run(String[] args) {
		...
		/*
		 * // TO REMOVE ArrayList authors; int
		 * moreThanThreeQuotationsThatBelongToSameAuthor = 0; int
		 * numOfAuthorsThatHasMoreThanThreeQuotations = 0;
		 */
		...
		for (int fold = 0; fold < numFolds; ++fold) {
			...
			LossAugmentedPerceptron alg = new LossAugmentedPerceptron(
					...
					true, averageWeights, LearnRateUpdateStrategy.NONE);

			alg.setListener(new EvaluateModelListener(devCorpusInput,
					devCorpusOutput, averageWeights));
			...
			for (int idx = 0; idx < inputs.getNumberExamples(); ++idx) {
				...
				/*
				 * //TO REMOVE authors = new ArrayList();
				 */
				...
				for (int j = 0; j < outputsSize; ++j) {
					...
					if (outputs[idx].getAuthor(j) != 0) {
						 ...
						 * 
						 * int numOfQuotationsOfThisAuthor = 0; for(int k = 0; k
						 * < outputsSize; ++k) if (outputs[idx].getAuthor(k) ==
						 * outputs[idx].getAuthor(j))
						 * ++numOfQuotationsOfThisAuthor;
						 * if(numOfQuotationsOfThisAuthor > 3) {
						 * moreThanThreeQuotationsThatBelongToSameAuthor +=
						 * numOfQuotationsOfThisAuthor;
		 ...
		 * "Quotations:                                                               "
		 * + evalAverage.getNumObjects()); System.out.println(
		 * "Authors that have more than 3 quotations:                                 "
		 * + numOfAuthorsThatHasMoreThanThreeQuotations); System.out.println(
		 * "Quotations that belong to the group of more than 3 quotations per author: "
		 * + moreThanThreeQuotationsThatBelongToSameAuthor); System.out.println(
		 * "Lost quotations:                                                          "
		 * + (moreThanThreeQuotationsThatBelongToSameAuthor -
		 * numOfAuthorsThatHasMoreThanThreeQuotations));
		 */
